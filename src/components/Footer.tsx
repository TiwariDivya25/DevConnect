...
//          <footer
//       className={`border-t ${
//         isDark ? "bg-gray-900 border-gray-800 text-gray-400" : "bg-gray-100 border-gray-300 text-gray-600"
//       }`}
//     >
...
export default function Footer() {
  ...
  const footerClasses = isDark 
    ? "relative bg-gradient-to-b from-gray-900 via-gray-900 to-black border-t border-cyan-900/30 text-gray-300 overflow-hidden"
    : "relative bg-gradient-to-b from-white via-gray-50 to-gray-100 border-t border-blue-200 text-gray-700 overflow-hidden";
    ...
    <>
      <footer className={footerClasses}>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          {/* Main Footer Grid - Reduced to 3 columns for compactness */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3 group">
                <div className="relative">
                  <div className={`absolute inset-0 bg-gradient-to-r ${isDark ? 'from-cyan-500 to-blue-500' : 'from-blue-500 to-indigo-500'} rounded-xl blur opacity-75 group-hover:opacity-100 transition-opacity duration-500`}></div>
                  <div className={`relative p-2 ${isDark ? 'bg-gray-900 border-cyan-500/20' : 'bg-white border-blue-200'} rounded-xl border group-hover:scale-105 transition-all duration-300 ${isDark ? 'group-hover:border-cyan-500/40' : 'group-hover:border-blue-300'}`}>
                    <Code2 className={`w-6 h-6 ${iconColor300} group-hover:scale-110 group-hover:rotate-12 transition-transform duration-300`} />
                  </div>
                </div>
                <div>
                  <h2 className={`text-xl font-bold bg-clip-text text-transparent ${textGradient}`}>
                    DevConnect
                  </h2>
                  <p className={`text-sm ${textColor}`}>Next Generation Dev Community</p>
                </div>
              ...
              <div className="flex gap-3 pt-2">
                {[
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className={`p-2 rounded-lg border ${social.color} transition-all duration-300 transform hover:scale-110 hover:-translate-y-1 ${
                      isDark 
                        ? 'bg-gray-800/50 border-gray-700 text-gray-400' 
                        : 'bg-gray-100 border-gray-300 text-gray-600'
                    }`}
                    ...
                    aria-label={social.label}
                  >
                    <social.icon className="w-4 h-4 group-hover:rotate-12 transition-transform duration-300" />
                  </a>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  { label: "Events", icon: Calendar, path: "/events" },
                  { label: "Profile", icon: User, path: "/profile" },
                  { label: "Docs", icon: Shield, path: "/docs" },
                  { label: "Contributors", icon: UsersIcon, path: "/contributors" },
                ].map((link, i) => (
                  <Link
                    key={i}
                    to={link.path}
                    className={`group p-2 rounded-lg border transition-all duration-300 hover:translate-x-1 hover:shadow-lg relative overflow-hidden ${cardClasses}`}
                  >
                    <div className={`absolute inset-0 bg-gradient-to-r ${isDark ? 'from-cyan-500/0 via-cyan-500/10 to-blue-500/0' : 'from-blue-500/0 via-blue-500/5 to-indigo-500/0'} opacity-0 group-hover:opacity-100 transition-opacity duration-500`}></div>
                    <div className="relative flex items-center gap-2">
                      <link.icon className={`w-4 h-4 ${iconColor} group-hover:scale-110 transition-transform duration-300`} />
            ...
            {/* Newsletter - Kept but simplified */}
            <div className="space-y-4">
              <h3 className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-gray-900'} flex items-center gap-2 group`}>
                <Rocket className={`w-5 h-5 ${iconColor} group-hover:rotate-45 transition-transform duration-300`} />
                Stay Updated
              ...
              <form onSubmit={handleSubscribe} className="space-y-3">
                <div className="relative group/input">
                  ...
                  <input
                    ...
                    className={`relative w-full pl-10 pr-4 py-2 ${inputBg} rounded-xl ${isDark ? 'text-white' : 'text-gray-900'} placeholder-gray-500 focus:outline-none focus:ring-2 focus:border-transparent transition-all duration-300 focus:shadow-lg z-10 ${
                      isDark 
                        ? 'focus:ring-cyan-500/50 focus:shadow-cyan-500/20'
                        : 'focus:ring-blue-500/50 focus:shadow-blue-500/20'
                    }`}
                ...
                <button
                  type="submit"
                  disabled={isSubmitting || subscriptionSuccess}
                  className={`w-full py-2 px-4 rounded-xl font-medium transition-all duration-300 flex items-center justify-center gap-2 relative overflow-hidden group/button ${
                    subscriptionSuccess 
                      ? 'bg-green-600 hover:bg-green-700' 
                      : isDark
                        ? 'bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-700 hover:to-blue-700'
          ...
          {/* Footer Bottom */}
          <div className={`pt-4 border-t ${borderColor} relative group/bottom`}>
            <div className={`absolute inset-0 bg-gradient-to-r from-transparent ${isDark ? 'via-cyan-500/10' : 'via-blue-500/5'} to-transparent opacity-0 group-hover/bottom:opacity-100 transition-opacity duration-500`}></div>
